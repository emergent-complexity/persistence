...
RANDOM_SEED = None            # Set to None for stochastic runs
...
FIELD_CONFIGS = {
    'carbon': {
        'decay': 0.01,      # Stable substrate, doesn't vanish instantly
        'diffusion': 0.08,   
        'init_value': 0.0
    },
    'waste': {
    ...
    'necromass': {
        'decay': 0.001,       
        'diffusion': 0.001,    # Very heavy, barely moves
        'init_value': 0.0
    }
}
...
SOURCES = [
    # A single, guaranteed central vent
    #{
    #    'field': 'carbon',
    #    'type': 'vent',
    #    'pos': (25, 25),
    ...
    # random vents scattered elsewhere
...
SPECIES_CONFIGS = {
    'standard': {
        ...
        'max_bite': 4.0,             # Max units of matter taken per turn
        'starting_energy': 20.0,     # Units of Energy an agent starts with
        ...
        'repro_prob': 0.10,          # Probability of reporduction if Mass and 
        ...
        # 5. --- DEATH PARAMETERS  ---
        'death_threshold_E': 0.0,    # Minumum energy limit
    ...
    'mutant': {
        ...
        # 1. --- MATTER INTERACTIONS ---
        # Substrates and efficiencies of energy extraction
        'intakes': {'waste': 0.8, 'necromass': 0.1}, 
        # Distribution secretions (MUST SUM TO 1.0)
        'excretions': {'carbon': 1.0},
        # Toxins and bio-accumulation multipliers
        'toxins': {'carbon': 0.05}, 
        ...
        'max_bite': 4.0,             # Max units of matter taken per turn
        'starting_energy': 20.0,     # Units of Energy an agent starts with
        'metabolic_cost': 1,         # Energy burned & heat released per step
        'entropy_coefficient': 0.5,  # Heat produced per unit of energy gained
        'growth_efficiency': 0.2,    # Percentage of intake mass stored
...
REPRO_COST_RATIO = 0.5  # Parent gives 50% of threshold energy to child
BASE_BODY_MASS = 2      # Mass of an agent
